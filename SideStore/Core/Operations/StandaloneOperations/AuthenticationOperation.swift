...
final class AuthenticationOperation: BaseStandaloneOperation<AuthenticatedOperationContext, AuthenticationResult>, @unchecked Sendable {
    ...
    private func silentSignIn() async throws -> (ALTAccount, ALTAppleAPISession)? {
        ...
        self.debugLog("""
            [AuthenticationOperation] Background credential visibility: \
            keychainService=\(Bundle.Info.appbundleIdentifier), \
            bundleID=\(Bundle.main.bundleIdentifier ?? "nil"), \
            hasEmail=\(appleID != nil), \
            hasPassword=\(password != nil), \
            hasADSID=\(adsid != nil), \
            hasXcodeToken=\(xcodeToken != nil)
            """)
